...
│  │  ┌──────────┐  ┌──────────┐  ┌─────────────────────┐  │  │
│  │  │ Install  │  │ Activate │  │  Fetch (network-    │  │  │
│  │  │ (cache   │  │ (clean   │  │  first + cache-fill)│  │  │
│  │  │ assets)  │  │ old caches)│  └─────────┬───────────┘  │  │
│  │  └──────────┘  └──────────┘  │           │              │
│  │                              │           ▼              │
│  │  ┌───────────────────────────┴──────────────────────┐  │
...
### On Fetch (Network-first + Cache-fill)
...
└────┬─────┘
     ...
     ▼
┌──────────┐     ┌──────────┐
│  fetch()  │────▶│ Success? │───yes──▶ Return response
│ (network) │     └────┬─────┘           + cache clone
└──────────┘          │ no
                      ▼
               ┌──────────────┐
               │  caches.match │───hit──▶ Return cached
               │  (fallback)   │
               └──────┬───────┘
                      │ miss
                      ▼
...
Successful network responses are **cloned and written to cache** in the background, so assets become available offline on subsequent visits.
...
sw.js (line 1):
────────────────────────────────────────────────────
const CACHE_VERSION = 2;   ← BUMP THIS on every deploy
────────────────────────────────────────────────────
```
